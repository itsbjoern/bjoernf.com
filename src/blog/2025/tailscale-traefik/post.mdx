...
2. Your Tailscale device is `device.tail-net.ts.net`
3. You have a domain `example.com` that you own and can configure DNS for.

The first step is to set up your desired DNS subdomains in your DNS provider to point at your tailnet device address. To allow all services to be forwarded to Tailscale I set up a wildcard CNAME record for `*.athome.example.com` pointing to `device.tail-net.ts.net`. This way any subdomain request will be routed to the Tailscale device. Any subdomain works here, I just chose the namespace `athome` to free up the other subdomains for other purposes.
