import { detect as detectBrowser } from './browser';
 ...
 */
const validate = function (options: Partial<Options> = {}) {
 ...
 */
const endpoint = function (server: string) {
...
export const detect = function () {
  ...
  const domainId = elem.getAttribute('data-ackee-domain-id') as string;
 ...
 * @param {String} server - URL of the Ackee server.
 * @param {?Object} validatedOptions
 * @returns {Object} instance
 */
export const create = function (server: string, options: Partial<Options>) {
  ...
  if (
    validatedOptions.ignoreLocalhost === true &&
  ...
  const _record = (
    domainId: string,
    attrs = attributes(validatedOptions.detailed),
    ...
    send(url, createRecordBody(domainId, attrs), validatedOptions, (json) => {
      ...
      const interval = setInterval(() => {
        ...
        send(url, updateRecordBody(recordId), validatedOptions);
      }, 15000);
  ...
  const _action = (
    ...
    send(url, createActionBody(eventId, attrs), validatedOptions, (json) => {
  ...
  const _updateAction = (actionId: string, attrs: Record<string, any>) => {
    ...
    send(url, updateActionBody(actionId, attrs), validatedOptions);
  };
